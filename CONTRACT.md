...
**Role:** Signed Rust extension compiled into the gNode daemon. Provides Content Management System capabilities — template rendering, content storage/caching, asset bundling, and message-format registration — exposed as **22 registered daemon commands** across four handler families (`src/handlers/{template,content,asset,format}.rs`). State lives entirely in ValKey; the extension itself is a stateless command dispatcher.

`name: cms` · `tier: base` · `version: 1.0.0` · `rust_feature: cms` (extension.yaml:16-28). Ships as a default base-tier extension; serves as the reference implementation for the gNode extension system.

---

## 1. PROVIDES

22 commands registered into the daemon's command dispatcher at init via each handler's `register()` (e.g. `template.rs:28-193`). Each command is invoked through the daemon's normal command path; parameters arrive as a JSON object (`Command{id, parameters}`), results are `CommandResult` (`{success(Value)|error(String)|success_json(String)}`, `template.rs:34,254-259`).
...
| Command | Required params | Optional params (defaults) | Returns |
|---|---|---|---|
| `register_format` | `format_definition: object` | — | `{status:registered, format_name:string}` (format.rs:29,45-54,92-159) |
| `list_formats` | — | — | `object \| array` (depends on Rust-native vs ValKey backend — see Adherence) (format.rs:30,55-64,213-316) |
| `detect_format` | `message: string` | — | `{format_name:string, version:string, confidence:number[0.0-1.0]}` (format.rs:31,65-74,318-429) |
| `convert_format` | `source_format: string`, `target_format: string`, `message: string` | `source_version: string=1.0.0`, `target_version: string=1.0.0` | `object` (converted message in target format) (format.rs:32,75-84,432-597) |
...
| Format processor (Rust-native) | `crate::daemon::GNodeDaemon::get_format_processor_ref()` | `#[cfg(feature="cms")]` fallback; ValKey fallback if absent | format.rs:113-115 |
...
Template `variables` as objects; `manifest` as object with nested arrays/objects (JSON-encoded into Lua HASH fields); capability `filters` as `{capability_name:[operator, threshold]}`; `format_definition` as nested JSON (template.rs:213-217; asset.rs:323-326; format.rs:105-108).
...
4. **Non-deterministic format-command response schema.** Rust-native processor returns `{status, format_name}` but the ValKey fallback returns `{status, result{...}, timestamp}`; unwrap logic assumes a `result` field (format.rs:117-124 vs 190-200). The Rust fallback is feature-gated (`#[cfg(feature="cms")]`) and may be absent at runtime; ValKey fallback's error handling is log-only.
