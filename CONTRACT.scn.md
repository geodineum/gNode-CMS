...
## ::ROLE
Signed Rust extension compiled INTO gNode daemon (gNode=Sun, ValKey=backend). The CMS organ: template render + content store/cache + asset bundle + message-format registry. Stateless command dispatcher; ALL state in ValKey, site-scoped. `name:cms` `tier:base` `feature:cms` (extension.yaml:16-28). Reference impl for the gNode extension system. 22 commands, 4 handler families.
...
## ::CONTRACT
- PROVIDES: 22 daemon commands (signatures+defaults in CONTRACT.md §1). Stable key schema `{site_id}:asset:*` / `:template:*` / `:gnode:bundle:*`. GNODE_ASSET_* fcall surface for the daemon background builder (bundle keys).
...
- format Rust-native fallback is feature-gated → may be absent at runtime; ValKey fallback error handling log-only (not fatal) AND divergent response wrapper (`{status,format_name}` vs `{status,result{},timestamp}`) — non-deterministic schema, unwrap assumes `result`.
...
PROVIDES_TO: gNode daemon command dispatcher (22 cmds) · daemon background bundle builder (reads `{site_id}:gnode:bundle:*`, written from manifests).
...
## ::LATENT
- "22 CMS commands, four handler families, register() into the daemon dispatcher at init"
...
- "format processor: Rust-native #[cfg(feature=cms)] fallback to ValKey, divergent response wrappers"
